...
type SlideProps = { reveals: number[] };
...
export const Title: React.FC<SlideProps> = ({ reveals }) => {
  const frame = useCurrentFrame();
  return (
    <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "100%" }}>
      <div style={revealAt(frame, reveals[0])}>
        <Eyebrow>{meta.courseTitle.toUpperCase()}</Eyebrow>
      </div>
      <div
        style={{
      ...
      >
        ...
        <span>LESSON 1 OF 5 &nbsp;·&nbsp; SELF STUDY &nbsp;·&nbsp; ACCOUNTING</span>
      </div>
    </div>
  );
};
